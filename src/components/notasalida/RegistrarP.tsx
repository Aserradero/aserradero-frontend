...
export default function RegistrarP() {
    ...
    const calidadEditor = (options: ColumnEditorOptions) => {
        return (
            <Dropdown
                value={options.value}
                options={calidad}
                onChange={(e: DropdownChangeEvent) => options.editorCallback!(e.value.name)}
                optionLabel="name"  
    ...
    const accept = () => {
        toast.current?.show({ severity: 'info', summary: 'Confirmado', detail: 'Se registraron los productos', life: 3000 });
    };
    

    const reject = () => {
        toast.current?.show({ severity: 'warn', summary: 'Denegado', detail: 'Realice los cambios necesarios ', life: 3000 });
    };
    ...
    const agregarProducto = async() => {
        if (value1 === null || value2 === null || value3 === null || value4 === null || !selectedCalidad) {
            toast.current?.show({
                ...
                detail: 'Por favor, complete todos los campos antes de registrar el producto.',
                life: 3000,
            });
    ...
    const eliminarProducto = (producto: any) => {
        const productosRestantes = productosRegistrados.filter(p => p !== producto);
  ...
  const registrarProductos = async () => {
    ...
    const productosARegistrar = productosRegistrados.map(producto => ({
        ...
        piesTabla: Number(((producto.ancho * producto.largo * producto.grosor * producto.cantidad) / 12).toFixed(1)),
    ...
    try {
        ...
        if (response.status === 201) {
            ...
            toast.current?.show({
                ...
                detail: "Todos los productos han sido registrados en la base de datos.",
                life: 3000,
            });
        ...
        <>
            ...
            <div className="card flex flex-wrap gap-3 p-fluid mt-2">
                <div className="flex-auto">
                    <label htmlFor="stacked-buttons" className="font-bold block mb-2">Grosor</label>
                ...
                <div className="flex-auto">
                    <label htmlFor="minmax-buttons" className="font-bold block mb-2">Ancho</label>
                    <div className="p-inputgroup flex-1">
                        <span className="p-inputgroup-addon">
                ...
                <div className="flex-auto">
                    <label htmlFor="horizontal-buttons" className="font-bold block mb-2">Largo</label>
                    <div className="p-inputgroup flex-1">
                        <span className="p-inputgroup-addon">
            ...
            <div className="card flex flex-wrap gap-3 p-fluid">
                <div className="flex-auto">
                    <label htmlFor="stacked-buttons" className="font-bold block mb-2">Calidad de la madera</label>
                    <div className="p-inputgroup flex-1">
                        <span className="p-inputgroup-addon">
                ...
                <div className="flex-auto">
                    <label htmlFor="stacked-buttons" className="font-bold block mb-2">Ingresar la cantidad de madera</label>
                    <div className="p-inputgroup flex-1">
                        <span className="p-inputgroup-addon">
            ...
            <div className="card">
            <DataTable value={productosRegistrados}
             editMode="row"
             dataKey="id"
            ...
            header="Productos Registrados">
                ...
                />           
            <Column header="Pies/Tabla"
            body={(rowData) => {
                // Realiza el cálculo
                const piesPorTabla = (rowData.ancho * rowData.largo * rowData.grosor * rowData.cantidad) / 12;
